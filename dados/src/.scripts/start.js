...
const AUTH_CREDS_FILE = path.join(QR_CODE_DIR, 'creds.json');
...
function setupGracefulShutdown() {
  const shutdown = () => {
    mensagem('🛑 Encerrando o Shania Yan... Até logo!');
    if (botProcess) {
...
async function displayHeader() {
  const header = [
    `${colors.bold}🚀 Shania Yan - Conexão WhatsApp${colors.reset}`,
...
async function hasSavedAuthState() {
  try {
    if (!fsSync.existsSync(AUTH_CREDS_FILE)) return false;
    const authFiles = await fs.readdir(QR_CODE_DIR);
    return authFiles.length > 0;
  } catch (error) {
    aviso(`⚠️ Não foi possível verificar a sessão salva: ${error.message}`);
    return false;
  }
}
...
async function main() {
  try {
    ...
    if (await hasSavedAuthState()) {
      mensagem('✅ Dados de autenticação encontrados. Tentando reutilizar a sessão salva.');
      startBot(false);
      return;
    }
