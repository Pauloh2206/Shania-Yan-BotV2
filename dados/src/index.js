...
const reportedDevErrors = new WeakSet();
...
// --- ADICIONE ESTAS LINHAS AQUI ---
  ...
  const devDebug = process.env.DEV_DEBUG === '1' || process.env.DEV_DEBUG === 'true' || config?.debug === true;
  ...
  const enviarErroParaDev = async (contexto, error, dados = {}) => {
    try {
      const devLogs = config?.devLogs;
      if (!devLogs?.enabled || !devLogs?.jid) return;
      if (error && typeof error === 'object') {
        if (reportedDevErrors.has(error)) return;
        reportedDevErrors.add(error);
      }

      const erroTexto = String(error?.message || error || 'Erro desconhecido').slice(0, 1200);
      const stack = String(error?.stack || 'Stack não disponível').slice(0, 1800);
      const mensagem =
        `🚨 *ERRO DO BOT*\n\n` +
        `📍 *Contexto:* ${contexto}\n` +
        `⚙️ *Comando:* ${dados.command || 'N/A'}\n` +
        `🆔 *Mensagem:* ${dados.msgId || 'N/A'}\n` +
        `📱 *Chat:* ${dados.from || 'N/A'}\n` +
        `👤 *Remetente:* ${dados.sender || 'N/A'}\n\n` +
        `❌ *Erro:*\n${erroTexto}\n\n` +
        `📄 *Stack:*\n${stack}`;

      const arquivoTxt = Buffer.from(mensagem, 'utf8');
      const nomeArquivo = `erro-${String(contexto).replace(/[^a-z0-9_-]/gi, '_')}-${Date.now()}.txt`;

      await nazu.sendMessage(devLogs.jid, {
        document: arquivoTxt,
        mimetype: 'text/plain',
        fileName: nomeArquivo,
        caption: `📄 Relatório de erro: ${contexto}`
      });
      console.log(`✅ Erro enviado para o Dev: ${contexto}`);
    } catch (envioError) {
      console.error('❌ Não foi possível enviar o erro para o Dev:', envioError.message);
    }
  };
  ...
  async function handleAutoDownload(nazu, from, url, info) {
    try {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', e, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
...
// ... aqui segue o seu switch (command) ...
  ...
  writeJsonFile(groupFile, groupData);
    ...
    debugLog('Parser de comando', {
      msgId,
      body: body.slice(0, 300),
      bodyNormalizado: budy2.slice(0, 300),
      prefixo: groupPrefix,
      isCmd,
      command,
      args,
...
Código: *${roleCode}*`,
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', dmError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', reactionError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
    ...
    const startRemindersWorker = (nazuInstance) => {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', err, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
    ...
    const scheduleGroupJob = (groupId, type, timeStr, nazuInstance) => {
      ...
      void enviarErroParaDev('captura-local', e, {
        ...
        command: typeof command !== 'undefined' ? command : null
            ...
            try { writeJsonFile(filePath, data); } catch (e) {
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', e, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      }); console.error('[Cron] Failed to write schedule run:', e); }
          } catch (e) {
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', e, {
        msgId,
    ...
    const getFileBuffer = async (mediakey, mediaType, options = {}) => {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', fileError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
    ...
    if (isGroup && isAntiPorn && !info.key.fromMe) {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', adminError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
  ...
  const botStateFile = pathz.join(DATABASE_DIR, 'botState.json');
    ...
    if (isGroup) {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', relError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
    ...
    if (!info.key.fromMe && isAssistente && !isCmd && ((_botShort && budy2.includes(_botShort)) || (menc_os2 && menc_os2 == await getBotNumber(nazu))) && KeyCog) {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', assistentError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
...
// -----------------------------------------------------------
    ...
    debugLog('Entrando no switch', { command, isCmd, from, msgId });
    switch (command) {
      ...
      case 'testeerrodev':
        if (!isOwner && !isSubOwner) {
          return reply('🚫 Apenas o dono ou subdono pode testar o alerta do Dev.');
        }

        try {
          const erroDeTeste = new Error('Este é um erro de teste do sistema de alertas do Dev.');
          await enviarErroParaDev('teste-manual-dev', erroDeTeste, {
            command,
            body,
            from,
            sender,
          ...
          await reply('✅ Erro de teste enviado para o Dev. Verifique o WhatsApp técnico.');
        } catch (error) {
          await enviarErroParaDev('falha-no-teste-manual-dev', error, {
            command,
            body,
            from,
            sender,
          ...
          await reply('❌ Não foi possível executar o teste de envio ao Dev.');
        }
        break;
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', listError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', sendError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', deleteErr, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', updateErr, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', mediaError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', unlinkErr, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', updateMediaError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', rmdirErr, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', cleanupError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      case 'rpg': {
        try {
          ...
          await sendMenuImage(menuText, mediaBuffer);
        } catch (e) {
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', e, {
        msgId,
    ...
    break;
      ...
      case 'updates':
        try {
          ...
          const AtualCom = await axios.get('https://api.github.com/repos/Pauloh2206/Shania-Yan-BotV2/commits?per_page=1', {
            headers: {
          ...
          if (AtualCom > total) {
            ...
            const TextZin = await VerifyUpdate('Pauloh2206/Shania-Yan-BotV2', AtualCom - total);
            await reply(TextZin);
          } else {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', verifyError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
...
📂 Backup foi preservado para segurança.`);
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', resumeError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
...
case 'comandos':
	case 'commands':
	    try {
	        debugLog('Case menu iniciada', {
	            command,
	            prefixo: prefix,
	            from,
	            msgId,
	            menuImageExists: fs.existsSync(__dirname + '/../midias/menu.jpg'),
	            menuVideoExists: fs.existsSync(__dirname + '/../midias/menu.mp4')
	        });
	        const menuImagePath = __dirname + '/../midias/menu.jpg';
        ...
        // Verifica se existe vídeo, se não, usa imagem
	        ...
	        debugLog('Case menu: mídia carregada', {
	            mediaPath,
	        ...
	        debugLog('Case menu: texto gerado', {
	            chars: menuText?.length || 0,
	            preview: String(menuText || '').slice(0, 120)
	        });
      ...
      void enviarErroParaDev('captura-local', error, {
        ...
        command: typeof command !== 'undefined' ? command : null
	        ...
	        await enviarErroParaDev('case-menu', error, {
	            command,
	            body,
	            from,
	            sender,
	        ...
	        debugLog('Case menu: falha detalhada', {
	            name: error?.name,
...
case 'revelar':
    ...
    writeJsonFile(groupFile, groupData);
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', parseError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', metaError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', writeError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', groupError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
...
case 'clima':
    try {
        ...
        if (!GEMINI_API_KEY) {
            await nazu.sendMessage(from, { react: { text: '❌', key: info.key } });
            await enviarErroParaDev('case-clima-configuracao', new Error('Chave da API Gemini não configurada.'), {
                command,
                body,
                from,
                sender,
...
case 'add':
    try {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', nameError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', linkError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
...
case 'roubar':
  ...
  break;
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', stickerError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
    ...
    break;
      ...
      void enviarErroParaDev('captura-local', e, {
        ...
        command: typeof command !== 'undefined' ? command : null
          ...
          try { scheduleGroupJob(from, 'open', normalizedTime, nazu); } catch (e) {
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', e, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      }); console.error('Erro ao agendar open cron:', e); }
      ...
      void enviarErroParaDev('captura-local', e, {
        ...
        command: typeof command !== 'undefined' ? command : null
          ...
          // (Re)agendar job em memória
          try { scheduleGroupJob(from, 'close', normalizedTime, nazu); } catch (e) {
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', e, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      }); console.error('Erro ao agendar close cron:', e); }
...
• ${prefix}rentalclean - Limpar logs antigos`;
    ...
    await enviarErroParaDev('processamento-geral', error, {
      command,
      body,
      from,
      sender,
...
function getDiskSpaceInfo() {
  try {
    ...
    if (platform === 'win32') {
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', winError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
      ...
      // DEV_LOG_INSTRUMENTED: todos os erros capturados são enviados ao Dev.
      void enviarErroParaDev('captura-local', unixError, {
        msgId,
        from,
        command: typeof command !== 'undefined' ? command : null
      });
